...
- `innseal.html`, `kas-telenet.html`, `MonitorINN-DDBB-DIMAR.html`, `sdm/index.html` — páginas **auto-contenidas** con su propio CSS embebido; NO cargan `styles.css` (sus clases chocan, p. ej. `.nav` en SDM). `kas-telenet.html` y `MonitorINN-DDBB-DIMAR.html` son confidenciales: tienen `noindex` y están excluidas de sitemap/robots, y no están enlazadas desde `index.html` ni en `staticwebapp.config.json` (se accede por URL directa).
- `MonitorINN-DDBB-DIMAR.html` — landing de cumplimiento de requerimientos (documento por entidad/licitación). El "chrome" del documento (header, badges, tablas, kickers) sigue la paleta navy/rosa + Montserrat vía CSS embebido propio. Las 4 capturas de la app (`.app-shell`, mockups reales de producto en tema oscuro) son intocables: no aplican la marca del sitio y tienen su propio `font-family` fijado para no heredar cambios del body. Si se piden más landings de este tipo, replicar el mismo patrón (recolorear chrome, no tocar mockups de producto).
